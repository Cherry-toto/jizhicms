...
class TemplateController extends CommonController
{
	...
	public function index(){
		...
		if(!$webapi){
			...
			if(!M('sysconfig')->find(['field'=>'template_config'])){
				M('sysconfig')->add(['title'=>'插件配置','field'=>'template_config','type'=>2,'data'=>$webapi,'typeid'=>0]);
	...
	function update(){
			...
			if($this->frparam('action',1)){
				...
				switch ($action) {
				    ...
				    case 'template-install':
						...
						if(!count($tpl)){
							JsonReturn(array('code'=>1,'msg'=>'请选择使用场景！'));
						}
					...
					case 'backup':
						...
						$this->toBackup();
						JsonReturn(array('code'=>0,'msg'=>'备份成功！'));
						break;
				    default:
				        # code...
				        break;
		...
		JsonReturn(array('code'=>1,'msg'=>'参数错误,请选择对应模板！'));
	}
